...
{% set show_thank_you = true %}                     {# Displays "Gracias <Customer Name>!" above bottom barcode #}
...
{% set quote_to_invoice = false %}                  {# Changes Cotización wording to Factura in Sales and in Sale Quotes (does not apply to Work Order Quotes) #}
		...
		{% if not parameters.page or parameters.page == 2 or not page_loaded %}
			...
			<div>
				...
				{% if show_transaction_item_count %}
					...
					{% endfor %}
					<p>Total artículos: {{ transaction_item_count }}</p>
				{% endif %}
				...
				{% if not parameters.gift_receipt %}
					...
					<p id="receiptThankYouNote" class="thankyou">
						{% if show_thank_you %}
							Gracias {% if Sale.Customer %}{{Sale.Customer.firstName}} {{Sale.Customer.lastName}}{% endif %}!
						{% endif %}
					</p>
				{% endif %}
...
{% macro no_tax_applied_text(Sale) %}
	...
	{% if(has_non_taxed_item == 'true') %}
		<p id="noTaxApplied" class="thankyou">* No se aplican Impuestos</p>
	{% endif %}
{% endmacro %}
...
{% macro store_receipt(Sale,parameters,options,Payment) %}
	<div class="store">
		...
		{% else %}
			<h2 class="paymentTitle">Pagos</h2>
	...
	{% if Line.Serialized %}
		{% for Serialized in Line.Serialized.Serialized %}
			<div class='line_serial'>
				N.º serie: {{ Serialized.serial }} {{ Serialized.color }} {{ Serialized.size }}
			</div>
		{% endfor %}
	{% endif %}
	...
	<h1 class="receiptTypeTitle">
		{% if Sale.calcTotal >= 0 %}
			...
			{% elseif Sale.voided == 'true' %}
				{% if options.invoice_as_title and options.print_layout %}
					<span class="hide-on-print">
				{% endif %}
						Recibo <large>ANULADO</large>
				{% if options.invoice_as_title and options.print_layout %}
					</span>
					<span class="show-on-print">Factura ANULADO</span>
				{% endif %}
			{% else %}
		...
		{% else %}
			...
			{% if options.invoice_as_title and options.print_layout %}
				...
				<span class="show-on-print">Recibo de reembolso</span>
			{% endif %}
		{% endif %}
	</h1>
{% endmacro %}
...
{% macro sale_details(Sale,options) %}
	<p id="receiptInfo" class="details">
		...
		{% else %}
			<span class="receiptTicketIdField">
				<span class="receiptTicketIdLabel">
					{% if options.sale_id_instead_of_ticket_number %}Ventas: {% else %}Ticket: {% endif %}</span>
		...
		{% if isVATAndRegistrationNumberOnReceipt() %}
			{% if Sale.Shop.vatNumber|strlen %}
				<span class="vatNumberField">
					<span class="vatNumberLabel">N.º IVA: </span>
			...
			{% if Sale.Shop.companyRegistrationNumber|strlen %}
				<span class="companyRegistrationNumberField">
					<span class="companyRegistrationNumberLabel">Número de registro: </span>
		...
		{% if Sale.Register %}
			<span class="receiptRegisterNameField"><span class="receiptRegisterNameLabel">Registro: </span><span id="receiptRegisterName">{{Sale.Register.name}}</span><br /></span>
		{% endif %}

		{% if Sale.Employee %}
			<span class="receiptEmployeeNameField"><span class="receiptEmployeeNameLabel">Empleado: </span><span id="receiptEmployeeName">{{Sale.Employee.firstName}}</span><br /></span>
		{% endif %}
		...
		{% if Sale.Customer %}
			...
			{% if not options.company_name_override or not Sale.Customer.company|strlen > 0 %}
				<span class="receiptCustomerNameField"><span class="receiptCustomerNameLabel">Cliente: </span><span id="receiptCustomerName">{{Sale.Customer.firstName}} {{Sale.Customer.lastName}}</span><br /></span>
			{% endif %}
			...
			{% if not options.show_customer_name_only %}
				...
				{% if options.show_full_customer_address and ContactAddress.address1 %}
					<span class="receiptCustomerAddressField"><span class="receiptCustomerAddressLabel">Dirección: </span>{{ ContactAddress.address1 }}{% if ContactAddress.city %}, {{ ContactAddress.city }}{% endif %}{% if ContactAddress.state %}, {{ ContactAddress.state }}{% endif %}{% if ContactAddress.zip %}, {{ ContactAddress.zip }}{% endif %}<br /></span>
				{% endif %}
			...
			{% if isVATAndRegistrationNumberOnReceipt() %}
				{% if Sale.Customer.vatNumber|strlen %}
					<span class="receiptCustomerVatField">
						<span class="receiptCustomerVatLabel">N.º IVA cliente: </span>
				...
				{% if Sale.Customer.companyRegistrationNumber|strlen %}
					<span class="receiptCustomerCompanyVatField">
						<span class="receiptCustomerCompanyVatLabel">Número de registro cliente: </span>
			...
			{% if options.show_customer_notes %}
				{% if Sale.Customer.Note.note|strlen > 0 %}
					<span class="receiptCustomerNoteField"><span class="receiptCustomerNoteLabel">Nota: </span>{{ Sale.Customer.Note.note|noteformat|raw }}<br /></span>
				{% endif %}
			{% endif %}
		{% endif %}
	</p>
			...
			<table class="saletotals totals">
				<tbody id="receiptSaleTotals">
					...
					{% if not options.discounted_line_items and Sale.calcDiscount > 0 %}
						<tr><td>Descuentos</td><td id="receiptSaleTotalsDiscounts" class="amount">-{{Sale.calcDiscount|money}}</td></tr>
					{% elseif not options.discounted_line_items and Sale.calcDiscount < 0 %}
						<tr><td>Descuentos</td><td id="receiptSaleTotalsDiscounts" class="amount">{{Sale.calcDiscount|getinverse|money}}</td></tr>
					{% endif %}
	...
	{% if Sale.completed == 'true' and not parameters.gift_receipt %}
					...
					{% for Payment in Sale.SalePayments.SalePayment %}
						{% if Payment.PaymentType.name != 'Cash' %}
							...
							{% if Payment.CreditAccount.giftCard == 'true' %}
								...
								{% if Payment.amount > 0 %}
									<tr>
										<td class="label">Cargo tarjeta regalo</td>
								...
								{% elseif Payment.amount < 0 and Sale.calcTotal < 0 %}
									<tr>
										<td class="label">Reembolso en trajeta regalo</td>
								...
								{% elseif Payment.amount < 0 and Sale.calcTotal >= 0 %}
									<tr>
										<td class="label">Adquisición tarjeta regalo</td>
										<td id="receiptPaymentsGiftCardValue" class="amount">{{Payment.amount|getinverse|money}}</td>
									</tr>
									<tr>
										<td class="label">Saldo</td>
							...
							{% elseif Payment.CreditAccount %}
								...
								<tr>
									...
									{% else %}
									<td class="label">Cargo en cuenta</td>
		...
		{% if options.show_customer_credit_account and Sale.Customer and not parameters.gift_receipt and not store_copy %}
			{% if Sale.Customer.CreditAccount and Sale.Customer.CreditAccount.MetaData.creditBalanceOwed > 0 or Sale.Customer.CreditAccount.MetaData.extraDeposit > 0 %}
				<h2 class="footerSectionTitle">Cuenta de tienda</h2>
				<table class="totals">
					{% if Sale.Customer.CreditAccount.MetaData.creditBalanceOwed > 0 %}
						<tr>
							<td width="100%">Saldo adeudado: </td>
			...
			{% if Sale.Customer.MetaData.getAmountToCompleteAll > 0 %}
				<table class="totals">
					<tr class="total">
						<td width="100%">Saldo restante: </td>
		...
		{% if Payment.archived == 'false' %}
			{% if Payment.ccChargeID > 0 and Payment.CCCharge.declined == 'false' %}
				<td class="label" width="100%">
					...
					<br>N.º tarjeta: {{Payment.CCCharge.xnum}}
					{% if Payment.CCCharge.cardType|strlen > 0 %}
						<br>Tipo: {{Payment.CCCharge.cardType}}
					{% endif %}
					{% if Payment.CCCharge.cardholderName|strlen > 0 %}
						<br>Titular: {{Payment.CCCharge.cardholderName}}
					{% endif %}
					{% if Payment.CCCharge.entryMethod|strlen > 0 %}
						<br>Entrada: {{Payment.CCCharge.entryMethod}}
					{% endif %}
					{% if Payment.CCCharge.authCode|strlen > 0 %}
						<br>Aprobación: {{Payment.CCCharge.authCode}}
					{% endif %}
	...
	{% if Sale.ShipTo %}
		<div class="shipping">
			...
			{% for Phone in Sale.ShipTo.Contact.Phones.ContactPhone %}{% if loop.first %}
				<p>Teléfono: {{Phone.number}} ({{Phone.useType}})</p>
			{% endif %}{% endfor %}
...
{% macro shipping_address(Customer,Contact) %}
	<p>
		...
		{% if Customer.company|strlen > 0 %}A/A:{% endif %} {{Customer.firstName}} {{Customer.lastName}}<br>
...
{% macro layaways(Customer,isTaxInclusive,parameters,options) %}
	{% if Customer.Layaways and Customer.Layaways|length > 0 %}
		<h2 class="footerSectionTitle">Arts. en reserva</h2>
		...
		<table class="layways totals">
			...
			{% if Customer.MetaData.layawaysAllDiscounts > 0 %}
				<tr>
					<td class="label" width="100%">Descuentos</td>
					<td class="amount">{{Customer.MetaData.layawaysAllDiscounts|getinverse|money}}</td>
				</tr>
			{% endif %}
			<tr>
				<td class="label" width="100%">Total impuestos</td>
...
{% macro specialorders(Customer,isTaxInclusive,parameters,options) %}
	{% if Customer.SpecialOrders|length > 0 %}
		<h2 class="footerSectionTitle" id="lineItemSectionSO">Pedidos especiales</h2>
...
{% macro workorders(Customer,parameters,options) %}
	{% if Customer.Workorders|length > 0 %}
		<h2 class="footerSectionTitle">Órdenes de trabajo abiertas</h2>
...
{% macro sale_cash_payment(Sale) %}
	...
	{% endfor %}
	{% if pay_cash == 'true' %}
		<tr><td class="label">Efectivo</td><td id="receiptPaymentsCash" class="amount">{{total|money}}</td></tr>
		<tr><td class="label">Cambio</td><td id="receiptPaymentsChange" class="amount">{{Sale.change|money}}</td></tr>
	{% endif %}
{% endmacro %}
